...
app = FastAPI(
    title="CineLake AI - RAG + MCP API", version="0.2.0"
)  # Inicializa o aplicativo FastAPI definindo o título e versão 0.2.0.
...
class Pergunta(
    BaseModel
):  # Define o schema Pydantic para o payload JSON recebido nas requisições da rota /ask.
    ...
    incluir_ferramentas: list[
        str
    ] = []  # Lista opcional de nomes de ferramentas MCP a serem consideradas.


def _detectar_ferramenta(
    pergunta: str,
) -> str | None:  # Define a função auxiliar para mapeamento baseado em palavras-chave da pergunta.
    """Mapeia a pergunta para uma ferramenta MCP apropriada."""  # Docstring descrevendo a detecção de intenção para ferramentas MCP.
    p = (
        pergunta.lower()
    )  # Converte a pergunta inteira para minúsculas para busca insensível a maiúsculas/minúsculas.
    if any(
        palavra in p for palavra in ["saúde", "status", "health", "plataforma"]
    ):  # Verifica se trata de saúde global da plataforma.
        return "get_platform_health"  # Retorna o identificador da ferramenta MCP de saúde da plataforma.
    elif any(
        palavra in p for palavra in ["pipeline", "falhou", "execução"]
    ):  # Verifica se trata de status de execução de pipelines.
        return "get_pipeline_status"  # Retorna o identificador da ferramenta MCP de status de pipelines.
    elif any(
        palavra in p for palavra in ["freshness", "atrasado", "atrasada"]
    ):  # Verifica se trata de atraso ou defasagem de dados.
        return "get_data_freshness"  # Retorna o identificador da ferramenta MCP de freshness de tabelas.
    elif any(
        palavra in p for palavra in ["qualidade", "falhas"]
    ):  # Verifica se trata de falhas e checagens de qualidade de dados.
        return "get_data_quality_failures"  # Retorna o identificador da ferramenta MCP de qualidade de dados.
    elif any(
        palavra in p for palavra in ["schema", "tabela"]
    ):  # Verifica se trata de estrutura ou esquema de tabelas do banco.
        return (
            "get_table_schema"  # Retorna o identificador da ferramenta MCP de schemas de tabelas.
        )
    return None  # Retorna None caso nenhuma correspondência com ferramentas seja encontrada.


@app.post(
    "/ask", summary="Pergunta à plataforma combinando RAG e MCP"
)  # Declara o endpoint POST na rota /ask da aplicação FastAPI.
def ask(
    pergunta: Pergunta,
) -> dict[str, Any]:  # Define a função controladora da rota /ask que recebe o objeto Pergunta.
    ...
    try:  # Inicia o bloco protegido de execução do fluxo de RAG e MCP.
        ...
        documentos = buscar_documentos_similares(
            pergunta.texto, top_k=pergunta.top_k
        )  # Executa a busca vetorial no pgvector.
        contexto_docs = [  # Monta a lista formatada de trechos e metadados dos documentos recuperados.
            {  # Abre dicionário para cada documento individual recuperado.
                ...
                "similaridade": doc[
                    "similaridade"
                ],  # Score de similaridade de cosseno retornado pelo embedding.
                "trecho": doc["conteudo"][
                    :500
                ],  # Recorta os primeiros 500 caracteres do conteúdo textual do documento.
        ...
        ferramenta = _detectar_ferramenta(
            pergunta.texto
        )  # Detecta se alguma ferramenta MCP deve ser chamada para a pergunta.
        if ferramenta:  # Se uma ferramenta válida foi identificada para a pergunta.
            resultado_ferramenta = invocar_ferramenta_mcp(
                ferramenta
            )  # Executa a chamada à ferramenta via cliente MCP.
        ...
        )  # Registra no log de aplicação a exceção completa com traceback.
        ...
        ) from exc  # Relança como erro HTTP para resposta ao cliente da API.

    finally:  # Bloco executado obrigatoriamente tanto em caso de sucesso quanto de falha.
        latencia_ms = (
            time.time() - inicio
        ) * 1000  # Calcula o tempo total decorrido em milissegundos.
        try:  # Bloco protegido para gravação do log de auditoria no banco de dados.
            ...
            logger.error(
                "Falha ao registrar log RAG: %s", log_exc
            )  # Emite mensagem de erro no log.


@app.get(
    "/rag/metrics", summary="Métricas do uso do RAG"
)  # Declara o endpoint GET na rota /rag/metrics para observabilidade.
def rag_metrics() -> dict[
    str, Any
]:  # Define a função controladora que retorna estatísticas agregadas do RAG.
    """Retorna métricas agregadas do RAG."""  # Docstring da rota /rag/metrics.
    try:  # Bloco protegido para extração das métricas.
        return (
            obter_metricas_rag()
        )  # Executa as agregações e retorna o dicionário com métricas e histórico.
        ...
        raise HTTPException(
            status_code=500, detail=str(exc)
        ) from exc  # Retorna código HTTP 500 com a mensagem do erro.
