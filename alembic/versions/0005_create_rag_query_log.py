...
down_revision: str | None = (
    "0004"  # Identificador da revisão anterior na árvore de versões do Alembic.
)
branch_labels: str | Sequence[str] | None = (
    None  # Rótulos de ramificação (branch labels) opcionais.
)
...
def upgrade() -> None:  # Define a função de avanço (upgrade) da migração de schema.
    ...
    op.create_table(  # Executa a operação DDL de criação da nova tabela no banco de dados.
        ...
        ),  # Coluna de chave primária inteira autoincrementável (BigInteger).
        sa.Column(
            "pergunta", sa.Text(), nullable=False
        ),  # Coluna de texto contendo a pergunta submetida pelo usuário.
        sa.Column(
            "documentos_recuperados", JSONB(), nullable=True
        ),  # Coluna JSONB armazenando os documentos retornados na busca vetorial.
        sa.Column(
            "ferramenta_mcp", sa.Text(), nullable=True
        ),  # Coluna textual armazenando o nome da ferramenta MCP acionada (se houver).
        sa.Column(
            "resultado_ferramenta", JSONB(), nullable=True
        ),  # Coluna JSONB contendo a resposta retornada pela ferramenta MCP.
        sa.Column(
            "latencia_ms", sa.Float(), nullable=True
        ),  # Coluna numérica em ponto flutuante registrando o tempo de resposta em milissegundos.
        sa.Column(
            "status_code", sa.Integer(), nullable=True
        ),  # Coluna inteira registrando o código de status HTTP da requisição.
        sa.Column(
            "erro", sa.Text(), nullable=True
        ),  # Coluna de texto contendo a mensagem ou rastreio de erro em caso de falha.
        sa.Column(
            "timestamp", sa.DateTime(timezone=True), server_default=sa.func.now(), nullable=False
        ),  # Coluna temporal com fuso horário registrando o momento do log.
...
def downgrade() -> None:  # Define a função de reversão (downgrade) da migração de schema.
    ...
    op.drop_table(
        "rag_query_log"
    )  # Executa a operação DDL de exclusão da tabela rag_query_log caso seja feito rollback.
